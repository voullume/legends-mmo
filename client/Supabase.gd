extends Node
## SUPABASE CLIENT (Phase 3) — the PUBLIC, client-facing adapter: email/password auth, the player's
## own `characters` row, and RLS-scoped own-inventory reads. Nothing privileged lives here
## (stabilization P6): every service-role method and server-only read is in server/ServerRepo.gd,
## which only the dedicated zone server instantiates — this class carries NO service_key field.
