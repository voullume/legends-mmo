...
func get_inventory_as(token: String) -> Dictionary:
	var r = await _http(HTTPClient.METHOD_GET, "/rest/v1/inventory?select=id,slot,rarity,bonus_stat,bonus_amt,primary_stat,primary_amt,ilvl,affixes,item_power,upgrade_level,reforge_count,set_id,equipped,locked,created_at&order=created_at.desc", "", PackedStringArray(), token)
	if r["code"] == 200 and r["data"] is Array:
		return {"ok": true, "items": r["data"]}
